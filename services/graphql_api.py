...
def get_liquidity_pair_details(pair_id):
    ...
    try:
        ...
        logging.debug("No pair details found for the given pair ID.")
        ...
        logging.error(f"Value error while parsing data: {val_err}")
    ...
    try:
        ...
        if 'data' in data and 'token' in data['data'] and data['data']['token'] is not None:
            ...
            if pair_ids:
                ...
                logging.debug("No pairBase ID found for this token.")
                return derived_eth, None
        else:
            # Handle case where the token data is not present in the response
            logging.debug("Token data not found in the response. The token might not be indexed yet.")
            derived_eth=0
            return derived_eth, None
